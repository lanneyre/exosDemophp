	...
	<section>
		...
		<fieldset>
			...
			function maj($text)
			{
				...
				return str_replace("ç", "&Ccedil;", ucwords(strtolower($text)));
			}
			echo "aecenas non REMI LANNEY çmetus erat. l'Etiam fermentum sodales. <hr>";
			echo maj("aecenas non REMI LANNEY çmetus erat. l'Etiam fermentum sodales. ");
			?>
		</fieldset>
	</section>
	<hr>
		...
		<fieldset>
			...
			function toucheCoule($h, $v)
			{
				...
				if (!is_string($h) || !is_int($v)) {
					...
					*/
					// $ligneOK = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j"];
					// if(!in_array($h, $ligneOK) || $v < 1 || $v > 10){
